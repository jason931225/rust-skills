...
A server constructs one client, one clock, one telemetry sink, and then needs that same instance in a dozen handlers. If `Clone` deep-copies the service, callers invent their own `Arc` and your type leaks into every signature. The Microsoft Pragmatic Rust Guidelines treat a service as a handle: `Clone` bumps a refcount, methods forward to `Inner`. That is an API convention, not the same claim as "use `Arc` when you share" (`own-arc-shared`).
