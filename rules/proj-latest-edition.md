...
changes such as the revised prelude and unsafe-attribute rules. Microsoft
Pragmatic Rust Guidelines (M-LATEST-EDITION) therefore set `edition` to the
newest stable when creating a crate. Staying on 2015 or 2018 does not widen the
crate graph and does not lower MSRV.
...
# rust-version is a separate knob; see proj-msrv-declare.
```
...
- Edition choice does **not** control which crates you may depend on, and it is **not** an MSRV lever. Interop is always "any edition may use any other." `package.rust-version` is how you pin the compiler (`proj-msrv-declare`).
